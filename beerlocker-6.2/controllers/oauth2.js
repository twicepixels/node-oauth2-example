...
server.exchange(oauth2orize.exchange.code(function (client, code, redirectUri, callback) {
    Code.findOne({value: code}, function (err, authCode) {
        ...
        authCode.remove(function (err) {
            ...
            sendToken(authCode.clientId, authCode.userId, callback);
        });
    });
}));
...
server.exchange(oauth2orize.exchange.password(function (client, username, password, scope, callback) {
    User.findOne({username: username}, function (err, user) {
        ...
        user.verifyPassword(password, function (err, isMatch) {
            ...
            // Success
            sendToken(client._id.toString(), user._id.toString(), callback);
        });
    });
}));
...
function uid(len) {
    ...
    for (var i = 0; i < len; ++i) {
        ...
        buf.push(chars[Math.floor(random) + min]);
    }
    return buf.join('');
}

function sendToken(clientId, userId, callback) {
    var token = new Token({
        value: uid(256),
        userId: userId,
        clientId: clientId
